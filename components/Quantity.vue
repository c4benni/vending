...
export default {
  ...
  render(h) {
    ...
    const btn = (d, c) => h('v-btn', d, c)
    const icon = (d, c) => h('v-icon', d, c)
      ...
      return btn(
        {
          ...
          staticClass: `${staticClass} p-2 h-full min-w-[48px]`,
    ...
    return div(
      {
        attrs: { ...scoping },
        staticClass: 'root fill-before-after overflow-hidden isolate',
        class: [{ focused: this.inputFocused, __disabled: this.disabled }]
      },
      [
        action({
          disabled: this.parseVmodel <= this.min,
          iconName: this.useChevron ? 'chevron-left' : 'minus',
          click: () => {
        ...
        action({
          ...
          iconName: this.useChevron ? 'chevron-right' : 'plus',
